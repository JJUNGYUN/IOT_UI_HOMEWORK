...
class Sensor_list(_list):
    
    def __init__(self,window,Room_name):
        super(Sensor_list, self).__init__(window)
        self.Room_name = Room_name
        
    def load_json(self):
        with open("device_default.json", 'r') as f:
            self.default = json.load(f)
        self.list = list(self.default['Sensor'].keys())

    def enter_event(self):
        if self.now == len(self.list):
            return True
        else:
            Append_sensor(self.window).append_menu(self.Room_name,self.list[self.now])
